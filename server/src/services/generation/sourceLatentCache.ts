 ...
 */
function cacheKey(audioPath: string, tempo?: number, pitch?: number): string {
 ...
 */
export function getCachedLatent(audioPath: string, tempo?: number, pitch?: number): Buffer | undefined {
  try {
    const key = cacheKey(audioPath, tempo, pitch);
 ...
 */
export function saveCachedLatent(audioPath: string, latent: Buffer, tempo?: number, pitch?: number): void {
  try {
    const key = cacheKey(audioPath, tempo, pitch);
    const filePath = join(cacheDir(audioPath), `${key}.raw`);
