...
-- MD Confluence V2 -- STORM / Trajectory Anchor hybrid solver
...
solver = {
    name        = "md_confluence_v2",
    display     = "MD Confluence V2",
    description = "STORM / Trajectory Anchor hybrid. Disagreement- and inertia-modulated latent blend between STORM stiffness-gated dispatch (STORK/DPM++3M) and Anchor stateful correction stack (inertia, concept lock, identity/tonal anchor, look-back, RMS servo).",
    nfe         = 1,
    order       = 1,
    needs_model = false,
    stateful    = true,
...
local function smoothstep(x, lo, hi)
    if hi <= lo then return (x >= hi) and 1.0 or 0.0 end
    local t = clamp((x - lo) / (hi - lo), 0.0, 1.0)
...
local function cosine_sim(a, b, n)
    local dot = vec_dot(a, b, n)
    local na, nb = vec_norm(a, n), vec_norm(b, n)
    return dot / (na * nb + EPSILON)
end
...
local function rms(a, n)
    ...
    return math.sqrt(s / math.max(n, 1) + EPSILON)
end
...
local _anc_tonal_ref_cent  = nil
local _anc_tonal_ref_bands = nil
...
local function reset_anchor_state(n)
    ...
    _anc_tonal_ref_cent  = nil
    _anc_tonal_ref_bands = nil
...
local function anchor_candidate(x, v_curr, sigma_curr, sigma_next, step_idx, n, p)
    ...
    if f_tonal and past_warmup then
        if _anc_tonal_ref_cent == nil and sigma_ratio <= anchor_sigma then
            _anc_tonal_ref_cent  = spectral_centroid(_anc_out, n)
            _anc_tonal_ref_bands = band_energy(_anc_out, n)
        elseif _anc_tonal_ref_cent ~= nil then
            local eff_str = tonal_str * sigma_ratio
            if eff_str > 1e-6 then
                local curr_centroid = spectral_centroid(_anc_out, n)
                local curr_bands    = band_energy(_anc_out, n)
                local drift_norm = (curr_centroid - _anc_tonal_ref_cent) /
                                   (math.abs(_anc_tonal_ref_cent) + EPSILON)
                ...
                for b = 1, 4 do
                    ref_total  = ref_total  + _anc_tonal_ref_bands[b]
                    curr_total = curr_total + curr_bands[b]
                end
                if ref_total > EPSILON and curr_total > EPSILON then
                    local bsize = math.floor(n / 4)
                    for b = 0, 3 do
                        local ref_ratio  = _anc_tonal_ref_bands[b + 1] / ref_total
    ...
    if f_rms then
        ...
        if cur_rms > rms_target then
            ...
            for i = 0, n - 1 do _anc_out[i] = _anc_out[i] * scale end
        end
    end
...
function sample(xt, vt_buf, schedule, n, model_fn)
    ...
    if verbose then
        print(string.format("[CONFLUENCE] Schedule: %d steps | Mix: %d | Mode: %s | RK: %s",
            n_steps, mix_amount, disagreement_mode, tostring(rk_order)))
    end
    ...
    for i = 1, n_steps do
        ...
        -- ── CANDIDATE B: ANCHOR ─────────────────────────────────────────
        local x_next_anchor = anchor_candidate(x, v_curr, sigma_curr, sigma_next, step_idx, n, p)
        ...
        if spec_guard > 0 and disagreement > 0.01 then
            ...
            -- proportional to disagreement. 4-band split same as tonal anchor.
            local bsize  = math.floor(n / 4)
            local atten  = disagreement * spec_guard  -- 0..~2 * 0..1
            for j = 0, n - 1 do
                local band = math.floor(j / bsize)
