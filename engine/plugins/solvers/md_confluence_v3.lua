...
-- MD Confluence V3 -- STORM / Trajectory Anchor hybrid solver
...
solver = {
    name        = "md_confluence_v3",
    display     = "MD Confluence V3",
...
local function rms_range(a, off, cnt)
    local s = 0.0
    for i = off, off + cnt - 1 do s = s + a[i] * a[i] end
    return math.sqrt(s / math.max(cnt, 1) + EPSILON)
end

local function rms(a, n) return rms_range(a, 0, n) end
...
local function spectral_centroid(a, off, cnt)
...
local function band_energy(a, off, cnt)
...
function sample(xt, vt_buf, schedule, n, model_fn)
    ...
    if verbose then
        print(string.format("[CONFLUENCE V3] Schedule: %d steps | B=%d NPB=%d | Mix: %d | Mode: %s | RK: %s",
