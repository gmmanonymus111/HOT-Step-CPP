...
- Repetitive GGML noise (`CUDA graph warmup`, `CUDA Graph id`, `ggml_backend_cuda_graph_compute`) is filtered from the console and the UI log stream but **still written to `ace_engine.log`** (index.ts:262-264, [server/src/routes/logs.ts:27-31](../../../server/src/routes/logs.ts)).
