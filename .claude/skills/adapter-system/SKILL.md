...
3. **Basin re-base applies ONCE PER STACK, first adapter only** (`engine/src/dit.h:565-586`). Applying it per adapter resets the running base on every merge and discards all earlier adapters — at β=1 only the last adapter survives. Merge mode only; the cache key deliberately drops rebase fields in runtime mode (`pipeline-synth.cpp:194-199`).
...
- **LoKR cross-base conversion — VALIDATED failure, UNVALIDATED fix.** Converting adapters to non-turbo XL bases FAILS even though the bases are ~99% weight-identical. Root cause is **basin-sensitivity** (the adapter delta lands in a different loss basin), NOT weight drift. Candidate fix: a **β·(S−T) basin nudge** — designed and implemented (`adapter-merge.h:78`, applied in `dit.h:578`) but **NOT validated**. A local doc claims XL→XL sibling-base nudging was "validated by ear", which conflicts with the lead's handoff — treat as **unvalidated; verify by ear before relying on it**. Never assume weight similarity implies adapter transferability.
